...
class Car(pg.sprite.Sprite):#좀비가 끝에 도달했을때 발사되는 잔디깎에 대한 Class
    def __init__(self, x, y, map_y):
        pg.sprite.Sprite.__init__(self)

        rect = tool.GFX[c.CAR].get_rect()#이미지 가져옴
    ...
    def update(self, game_info):#자동차 움직임에 따른 업뎃
        self.current_time = game_info[c.CURRENT_TIME]
        if self.state == c.IDLE:
            pass
        elif self.state == c.WALK:
            self.rect.x += 4 #자동차 속도 늘릴수록 빨리짐, 자동차 움직임도 WALK로 저장한듯
    ...
    def draw(self, surface):#배경 관련된 함수, 지우니까 카드 다고르고 게임 시작시 화면 터짐
        surface.blit(self.image, self.rect)

class Bullet(pg.sprite.Sprite):#식물이 쏘는 탄환에 관한 Class
    def __init__(self, x, start_y, dest_y, name, damage, ice):
        ...
        self.rect.y = start_y
        self.dest_y = dest_y
        self.y_vel = 4 if (dest_y > start_y) else -4 #뭔지 잘 모르겠음
        self.x_vel = 4 #총탄 날아가는 속도, 줄이니까 느리게 날아감
        self.damage = damage
        self.ice = ice
        self.state = c.FLY
        self.current_time = 0

    def loadFrames(self, frames, name):#이미지 불러오기
    ...
    def load_images(self):#총알이 터지며 나오는 이미지를 로드해줌
        self.fly_frames = []
        self.explode_frames = []
        
        fly_name = self.name
        if self.name == c.BULLET_MUSHROOM:
            explode_name = 'BulletMushRoomExplode'#보라터짐
        else:
            explode_name = 'PeaNormalExplode'#초록터짐
    ...
    def update(self, game_info):
        ...
        if self.state == c.FLY:
            ...
            if self.rect.x > c.SCREEN_WIDTH:
                self.kill()
        elif self.state == c.EXPLODE:#체리폭탄 배치후 터지는 시간
            if(self.current_time - self.explode_timer) > 500:
                self.kill()

    def setExplode(self):#자폭환경설정
...
class Plant(pg.sprite.Sprite):#모든 식물에 관한 클래스
    def __init__(self, x, y, name, health, bullet_group, scale=1):#식물에 대한 정보 초기화
        pg.sprite.Sprite.__init__(self)
    ...
    def loadFrames(self, frames, name, scale, color=c.BLACK):#식물 이미지 불러오기
        frame_list = tool.GFX[name]
        if name in tool.PLANT_RECT:
            data = tool.PLANT_RECT[name]
            x, y, width, height = data['x'], data['y'], data['width'], data['height']
    ...
    def handleState(self):#식물 상태에 따라 다르게 적용
        if self.state == c.IDLE:
            self.idling()
    ...
    def animation(self):
        ...
        self.image = self.frames[self.frame_index]
        if(self.current_time - self.hit_timer) >= 200:#식물이 좀비에게 맞으면서 깜빡거리는 시간
            self.image.set_alpha(255)
        else:
            self.image.set_alpha(192)

    def canAttack(self, zombie):#좀비를 때릴 수 있는가
    ...
    def setIdle(self):#평화로움 상태
        self.state = c.IDLE
        self.is_attacked = False

    def setSleep(self):#식물도 잠을 잔다(버섯은 낮에 잠을 잠)
        self.state = c.SLEEP
        self.changeFrames(self.sleep_frames)

    def setDamage(self, damage, zombie):#식물이 좀비에게 맞을때 데미지 입음
        self.health -= damage
        self.hit_timer = self.current_time
        if self.health == 0:#식물죽음
            self.kill_zombie = zombie

    def getPosition(self):#식물 놓는 것
        return self.rect.centerx, self.rect.bottom

class Sun(Plant):#떨어지는 햇빛에 대한 클래스
    def __init__(self, x, y, dest_x, dest_y, is_big=True):
        if is_big:#햇빛 크기, 숫자 키우면 커짐
            scale = 0.9
            self.sun_value = c.SUN_VALUE
        else:#해 버섯이 떨어트리는 작은 햇빛
            scale = 0.6
            self.sun_value = 12
        Plant.__init__(self, x, y, c.SUN, 0, None, scale)#햇빛에 대한 정보 초기화
        self.move_speed = 1 #해가 떨어지는 속도
        self.dest_x = dest_x
        self.dest_y = dest_y
        self.die_timer = 0

    def handleState(self):#햇빛이 떨어지기 시작하는 위치, 햇빛의 수명에 관련된 함수
        ...
        if self.rect.centerx == self.dest_x and self.rect.bottom == self.dest_y:
            if self.die_timer == 0:#햇빛의 수명(일정시간 이상 지나면 사라짐)
                self.die_timer = self.current_time
            elif(self.current_time - self.die_timer) > c.SUN_LIVE_TIME:
                self.state = c.DIE
                self.kill()

    def checkCollision(self, x, y):#햇빛의 수명이 다해서 없어질건지 체크
        if self.state == c.DIE:
            return False
...
class PeaShooter(Plant):#기본적인 공격몹
    def __init__(self, x, y, bullet_group):#체력은 init에 있음
        Plant.__init__(self, x, y, c.PEASHOOTER, c.PLANT_HEALTH, bullet_group)
        self.shoot_timer = 0
        
    def attacking(self):#peashooter의 공속, 공격력
...
class RepeaterPea(Plant):#한라인에 여러발 쏘는 식물
    def __init__(self, x, y, bullet_group):
        Plant.__init__(self, x, y, c.REPEATERPEA, c.PLANT_HEALTH, bullet_group)
        self.shoot_timer = 0

    def attacking(self):
        if (self.current_time - self.shoot_timer) > 2000:#공격속도, 공격력, self.bullet_group.add을 늘리면 공격횟수 늘어남
            self.bullet_group.add(Bullet(self.rect.right, self.rect.y, self.rect.y,
                                    c.BULLET_PEA, c.BULLET_DAMAGE_NORMAL, False))
...
class ThreePeaShooter(Plant):#3라인에 한발씩 쏘는 식물
...
class SnowPeaShooter(Plant):#얼음 발사 식물
    def __init__(self, x, y, bullet_group):
        Plant.__init__(self, x, y, c.SNOWPEASHOOTER, c.PLANT_HEALTH, bullet_group)
        self.shoot_timer = 0

    def attacking(self):#BULLET_PEA_ICE의 성질이 중요함
        if (self.current_time - self.shoot_timer) > 2000:
            self.bullet_group.add(Bullet(self.rect.right, self.rect.y, self.rect.y,
                                    c.BULLET_PEA_ICE, c.BULLET_DAMAGE_NORMAL, True))
            self.shoot_timer = self.current_time

class WallNut(Plant):#호두는 호두의 hp에 따른 이미지가 각기 다름
    ...
    def load_images(self):#호두 깨지는 이미지 로딩
...
class CherryBomb(Plant):#체리폭탄
    def __init__(self, x, y):#hp를 WALLNUT_HEALTH로 공유, bombtimer까지 초기화
        Plant.__init__(self, x, y, c.CHERRYBOMB, c.WALLNUT_HEALTH, None)
        self.state = c.ATTACK
        self.start_boom = False
        self.bomb_timer = 0
        self.explode_y_range = 1#그 칸에만 영향줌
        self.explode_x_range = c.GRID_X_SIZE#터지는 범위
    
    def setBoom(self):#폭탄설치
    ...
    def animation(self):#점점 커지면서 폭발함
        if self.start_boom:
...
class Chomper(Plant):#좀비먹는 식물
    ...
    def canAttack(self, zombie):#소화하고 있지 않으면 공격가능
    ...
    def attacking(self):
        if self.frame_index == (self.frame_num - 3):#먹히면 좀비 없어지도록 설정.
            self.zombie_group.remove(self.attack_zombie)
        if (self.frame_index + 1) == self.frame_num:#먹고나서 바로 DIGEST로 이미지 넘어가도록 숫자 1당 입질 한번씩 증가
            self.setDigest()

    def digest(self):#소화에 관한 함수
...
class PotatoMine(Plant):
    ...
    def idling(self):
        if self.is_init:
            ...
            elif (self.current_time - self.init_timer) > 150:
                self.changeFrames(self.idle_frames)
                self.is_init = False
